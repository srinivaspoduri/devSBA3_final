...
public class PoliticalPartyExceptionTest {
	...
	@Test
	public void testAddPartyWithInvalidData() throws Exception {
		
		PartyDto politicalPartyDto = MasterData.getPartyDto();
		politicalPartyDto.setPoliticalPartyId(null);;
		when(this.partyService.registerParty(politicalPartyDto))
				.thenThrow(new InvalidDataException("Invalid data Recived to register new Party"));
		RequestBuilder requestBuilder = MockMvcRequestBuilders.post("/politics/api/v1/party/register-party")
				.content(MasterData.asJsonString(politicalPartyDto)).contentType(MediaType.APPLICATION_JSON)
				.accept(MediaType.APPLICATION_JSON);
		...
		System.out.println("***********************"+result.getResponse().getContentAsString());
		testAssert(currentTest(),
				(result.getResponse().getContentAsString().contains("Invalid data Recived to register new Party") ? "true" : "false"),
				exceptionTestFile);
